...
//- ----------------------------- model -------------------------------
modelFile = './assets/model/wg01/t-rex_v2.glb'
modelScale = 0.35
modelPos = { x: 0, y: -1.2, z: 0 }
...
btnNodes = []
btnObjs = []
addBtnFn('01', '', { x: -0.7, y: -0.2, z: 0 }, { x: 6, y: 0, z: -3 }, 1) //뒷발
// addBtnFn('02', '', { x: 1.5, y: 0.5, z: -0.1 }, { x: 5, y: 0.5, z: 0 }, 1) //이빨
// addBtnFn('03', '', { x: 0.9, y: -0.2, z: 0 }, { x: 6, y: 0, z: -3  }, 2) //앞다리
// addBtnFn('04', '', { x: -0.3, y: -0.95, z: 0 }, { x: 6, y: 0, z: -2 }, 3) //뒷발톱
//* ===================================================================
//*                               Core                                 
//* ===================================================================
//- --------------------------- initialFn -----------------------------
export function initialFn() {
    ...
    hLight = new THREE.HemisphereLight('#e2e296', 5)
    scene.add(hLight)

    dLight = new THREE.DirectionalLight('#ffffff', 5)
    ...
    dLight2 = new THREE.DirectionalLight('#e2e296', 3)
    dLight2.position.set( -1.5, 0.5, 2 );
    scene.add(dLight2)
...
//         modelObj2.name = 'modelObj'
//         modelObj2.scale.set(modelScale, modelScale, modelScale)
//         modelObj2.position.set(modelPos.x, modelPos.y, modelPos.z)
...
function addBtnFn(btnNum, tippyTitle, btnPos, camMovePos, aniIndex) {
    ...
    btnNode.addEventListener('click', () => {
        ...
        gsap.to(camera.position, { x: camMovePos.x, y: camMovePos.y, z: camMovePos.z, duration: 2, ease: 'Power2.easeInOut', onComplete: () => { } })
        // gsap.to(modelObj.position, { z: '-=0.5', duration: 2, ease: 'Power1.easeOut', onComplete: () => { } })
        // gsap.to(modelObj2.position, { y: '+=0.2', duration: 1, ease: 'Power1.easeOut', onComplete: () => { } })
        let modal = document.querySelector(`#modal${btnNum}`)
        modal.style.display = 'flex'
        gsap.to(modal, {
            left:"30%", top: "50%", opacity: 1, duration: 2, ease: 'Power3.easeOut',
            onComplete: () => {
                ...
                modalClose.addEventListener('click', () => {
                    ...
                    gsap.to(modal, {
                        left: "0px", opacity: 0, duration: 2, ease: 'Power3.ease',
                        onComplete: () => { modal.style.display = 'none' }
                    })
                    gsap.to(camera.position, {
                        x: camInitPos.x, y: camInitPos.y, z: camInitPos.z, duration: 2, ease: 'Power2.easeInOut',
                        onComplete: () => {
...
//     window.parent.document.querySelector('.rano_scream').style.display = "none";
// })
// content들 iframe html파일 안에 할지 말지 고민좀...
...
// document.querySelector("#wgBtn02").addEventListener("click", function () {
//     setTimeout(function () {
//         const audio = new Audio('./assets/audio/scream01.mp3')
//         audio.play();
//     }, 1000)
// })
